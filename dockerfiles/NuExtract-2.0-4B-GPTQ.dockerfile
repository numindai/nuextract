#!/usr/bin/env -S docker build . --secret id=hf_token,src=${HOME}/.cache/huggingface/token --build-arg CPU=1 -t numind_nuextract-2_0-4b-gptq.cpu --file
...
RUN --mount=type=secret,id=hf_token \
    ...
    hf download numind/NuExtract-2.0-4B-GPTQ \
        model.safetensors && \
...
if rclone config dump --quiet | jq -e '. | length > 0' >/dev/null; then
    ...
    echo "$(cat /tmp/vllm_version)/numind_nuextract-2_0-4b-gptq" >> /tmp/remote
...
python3 -m vllm.entrypoints.openai.api_server \\
    --model numind/NuExtract-2.0-4B-GPTQ \\
